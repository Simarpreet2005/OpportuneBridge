...
const isRecruiterOrAdmin = async (req, res, next) => {
    try {
        ...
        if (user.role !== "recruiter" && user.role !== "admin" && user.role !== "superadmin") {
            return res.status(403).json({
                message: "Access denied. Recruiters, Admins, or Superadmins only.",
                success: false
            });
        }
...
export default isRecruiterOrAdmin;
