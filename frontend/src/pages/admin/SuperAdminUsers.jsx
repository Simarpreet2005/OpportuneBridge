...
const SuperAdminUsers = () => {
    ...
    const handleSuspendUser = async (userId, suspend) => {
        try {
            setLoading(true);
            const res = await axios.patch(`${ADMIN_API_END_POINT}/users/${userId}/suspend`,
                { suspend },
                            ...
                            users.length <= 0 ? <TableRow><TableCell colSpan={6} className="text-center">No users found.</TableCell></TableRow> : (
                                users.map((user) => (
                                    <TableRow key={user._id}>
                                        ...
                                        <TableCell>
                                            <Badge variant={user.role === 'recruiter' ? 'secondary' : user.role === 'admin' ? 'destructive' : 'outline'}>{user.role}</Badge>
                                        </TableCell>
                                        <TableCell>
                                            {user.isSuspended ? (
